...
Public Function PrintProject()
    ...
    workbookName = ActiveWorkbook.Name           'ActiveProjName
    If ProtectedVBProject(Workbooks(workbookName)) = True _
                                                     Or HasProject(Workbooks(workbookName)) = False Then
    ...
    'document
    For Each vbcomp In Workbooks(workbookName).VBProject.VBComponents
        If vbcomp.Type = vbext_ct_Document Then
            For Each ws In Workbooks(workbookName).Worksheets
    ...
    ThisWorkbook.Sheets("PRINTER").Range("B1:B" & UBound(tmpString) + 1).Value = _
                                                                               WorksheetFunction.transpose(tmpString)
    
    If CodePrinter = False Then
        Exit Function
    End If
...
Sub FormatColourFormatters()
    With ThisWorkbook.Sheets("TXTColour")
        uPrinter.LBLcolourCode.ForeColor = .Range("GeneralFontBackground").Value
        uPrinter.LBLcolourKey.ForeColor = .Range("ColourKeywords").Value
        uPrinter.LBLcolourOdd.BackColor = .Range("OddLine").Value
        uPrinter.LBLcolourComment.ForeColor = .Range("ColourComments").Value
    End With
End Sub
...
Sub copyLogo()
    ...
    With ThisWorkbook.Sheets("PRINTER")
        ...
        With .Range("B1")
            ...
            .Value = "vbaCodeArchive ~ CodePrinter" & vbNewLine & "Anastasiou Alex"
...
Sub PrintPDF()
    ...
    Dim fileName As String
    fileName = Left(ActiveProjName, InStr(1, ActiveProjName, ".") - 1)
    ...
    If uPrinter.chOpenFolder.Value = True Then FollowLink saveLocation
    If uPrinter.chOpenPDF.Value = True Then FollowLink filePath & ".pdf"
End Sub
...
Sub FollowLink(folderPath As String)
    ...
    For Each Wnd In oShell.Windows
        ...
        End If
    Next Wnd
    Application.ThisWorkbook.FollowHyperlink Address:=CStr(folderPath), NewWindow:=True
End Sub
